...
Here is an example of a map file, `example_map`, available in the STEAM GitHub repository:
```{r}
head(example_map)
```
...
Below is an example of a matrix of admixture proportions, `example_props`, available in the STEAM GitHub repository. This matrix contains the admixture proportions for `n` = 1000 admixed individuals and `K` = 2 ancestral populations:
```{r}
head(example_props)
```
...
`g`, the number of generations since admixture, is a required parameter for both `get_thresh_analytic` and `get_thresh_simstat`. `g` can be quickly estimated using the `get_g` function.
...
Store this local ancestry correlation in a data frame with three columns, as in the following examples (available in the STEAM GitHub repository):
